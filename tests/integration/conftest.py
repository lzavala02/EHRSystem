...
These fixtures reset mutable in-memory API state before every test so each test
executes against a predictable baseline and does not leak state to others.
"""
...
_BASELINE_REPORT_ACCESS_TOKENS = deepcopy(api.REPORT_ACCESS_TOKENS)
_BASELINE_SECURE_MESSAGES = deepcopy(api.SECURE_MESSAGE_PAYLOADS)
...
def reset_api_state() -> None:
    """Restore mutable in-process state to an import-time baseline per test."""
    ...
    api.REPORT_ACCESS_TOKENS.clear()
    api.REPORT_ACCESS_TOKENS.update(deepcopy(_BASELINE_REPORT_ACCESS_TOKENS))

    api.SECURE_MESSAGE_PAYLOADS.clear()
    api.SECURE_MESSAGE_PAYLOADS.extend(deepcopy(_BASELINE_SECURE_MESSAGES))
